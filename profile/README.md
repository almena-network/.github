...
| **cli** | Terminal UI (TUI) client built with Ratatui. Connects to the daemon via gRPC for operators who prefer the command line. Includes the **Homebrew tap** (`brew tap almena-network/cli`) for the macOS Apple Silicon binary from GitHub Releases. |
...
- The **CLI** repo’s Homebrew formula only packages the `almena` binary for macOS; it does not ship the daemon or desktop app.
